...
def accept_contract():
    try:
        contract_id= contract_view.focus()
        if not contract_id:
            messagebox.showerror("Error", "Please select a contract first")
            return
        ...
        if response.status_code == 200:
            tk.messagebox.showinfo(title='Information', message='Contract accepted')
            result=response.json()
            refresh_player_summary()
        else:
            result=response.json()
            tk.messagebox.showerror(title='Error accepting contract', message=f'Contract has already been accepted')
            refresh_player_summary()
        ...
        tk.messagebox.showerror(title='Error', message='Error accepting contract')
...
def fulfill_contract():
    ...
    try:
        response=requests.post(
        f"https://api.spacetraders.io/v2/my/contracts/{contract_id}/fulfill",
        headers={
          "Authorization": f"Bearer {player_token.get()}",
        }
    )
        if response.status_code==200:
            result=response.json()
            tk.messagebox.showinfo(title='Information', message=f'Contract accepted with code {result}')
        else:
            result=response.json()
            tk.messagebox.showerror(title='Error accepting contract', message=f'Contract conditions have not been fulfilled')
       
    except  ConnectionError as ce:
       
        result=response.json()
        tk.messagebox.showerror(title='Error', message=f'Error accepting contract{result}')
   

def open_contract_options():
    contract_window = tk.Toplevel(root)
    contract_window.title('Contract options')
    contract_window.geometry('400x200')
   
    ttk.Label(contract_window, text='Contract Actions').grid(
        row=0, column=0
    )
   
    ttk.Button(contract_window,text='Accept Contracts', command=accept_contract).grid(
        row=1, column=0, sticky=tk.EW
    )
   
    ttk.Button(contract_window,text='Fulfill contract', command=fulfill_contract).grid(
        row=2, column=0, sticky=tk.EW
    )
 
    contract_window.columnconfigure(0,weight=1)


def open_ship_options():
    ship_window = tk.Toplevel(root)
    ship_window.title('Contract options')
    ship_window.geometry('400x200')
   
    ttk.Label(ship_window, text='Contract Actions').grid(
        row=0, column=0
    )
   
    ttk.Button(ship_window,text='Orbit ship', command=orbit_ship).grid(
        row=0, column=0, sticky=tk.EW
    )
   
    ttk.Button(ship_window,text='Dock ship', command=dock_ship).grid(
        row=1, column=0, sticky=tk.EW
    )
    ttk.Button(ship_window,text='Create Chart', command=create_chart).grid(
        row=2, column=0, sticky=tk.EW
    )
 
    ship_window.columnconfigure(0,weight=1)

def dock_ship():
    ship_symbol= ship_view.focus()
    if not ship_symbol:
        messagebox.showerror("Error", "Please select a ship first")
        return
    try:
        response=requests.post(
        ...
        if response.status_code==200:
            result=response.json()
            tk.messagebox.showinfo(title='Information', message=f'Ship docked')
        else:
            result=response.json()
            tk.messagebox.showerror(title='Error docking ship', message=f'Error {result}')
       
    except  ConnectionError as ce:
       
        result=response.json()
        tk.messagebox.showerror(title='Error', message=f'Error docking ship{result}')
...
def orbit_ship():
        ...
        try:
            ...
            if response.status_code==200:
                result=response.json()
                tk.messagebox.showinfo(title='Information', message=f'Ship in orbit')
            else:
                result=response.json()
                tk.messagebox.showerror(title='Error orbiting ship', message=f'Error {result}')
       
        except  ConnectionError as ce:
       
            result=response.json()
            tk.messagebox.showerror(title='Error', message=f'Error orbiting ship{result}')
...
def create_chart():
        ...
        try:
            response=requests.post(
            f"https://api.spacetraders.io/v2/my/ships/{ship_symbol}/chart",
            headers={
            "Authorization": f"Bearer {player_token.get()}"
        }
    )
            if response.status_code==200:
                result=response.json()
                tk.messagebox.showinfo(title='Information', message=f'Chart created')
            else:
                result=response.json()
                tk.messagebox.showerror(title='Error Creating Chart', message=f'Waypoint already charted')
       
        except  ConnectionError as ce:
       
            result=response.json()
            tk.messagebox.showerror(title='Error', message=f'Error creating chart{result}')
...
ttk.Button(player_summary, text='Contract options', command=open_contract_options).grid(
   
    row=4, column=0, columnspan=2, sticky=tk.EW
)

ttk.Button(player_summary, text='Ship options', command=open_ship_options).grid(
